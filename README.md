...
```text
.
├── data/raw/      # 부산 공공데이터 원본 CSV
├── notebooks/     # 수집, 전처리, 리뷰 태깅, 이미지 유사도 분석 노트북
...
`data/raw/`에는 부산 음식점, 관광, 축제, 전시 관련 공공데이터 CSV를 정리했습니다.

- 부산광역시 구별 음식점/식품접객업 현황
- 부산 안심식당, 모범음식점, 택슐랭 선정 식당 정보
- 부산맛집, 도보여행, 축제, 전시 국문 정보

파일별 세부 목록은 [`data/README.md`](data/README.md)에서 확인할 수 있습니다.
...
- **데이터 수집 및 전처리**: 부산 음식점 정보 및 네이버 음식점 리뷰/사진 크롤링 후 전처리
- **돼지국밥 특징 추출 모델 구현**: 리뷰 데이터를 기반으로 Gemini API와 Langchain을 이용하여 음식점의 특징을 라벨링하는 모델 구현 
- **국밥 사진 기반 유사도 판단 모델 구현**: CLIP을 이용하여 국밥 사진들 간의 유사도를 측정하는 모델 구현
